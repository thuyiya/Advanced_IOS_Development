...
5. lets call this on home controller
```swift
private var trip: Trip? {
        didSet {
            guard let user = user else { return }
...
```
7. lets create pickupLabel and button
```swift
class PickupController: UIViewController {
    ...
    func configureUI() {
        ...
        pickupLabel.anchor(top: mapView.bottomAnchor, paddingTop: 32)
...
```
8. lets add pick up annottion to the map
```swift
...
```
10. lets make the delegate to update trip object in home controller
```swift
...
```
11. lets make the delegateion in home
```swift
